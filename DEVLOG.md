...
| 항목 | 내용 |
|---|---|
| **현재 위치** | M2(핵심 기능 확장) 진행 중 — 태그 기능(세션 1) 완료, 세션 2(검색+일기 수정 화면) 착수 예정 |
| **완료** | Prisma 스키마 및 마이그레이션 · Express 서버 · 회원가입/로그인 API · JWT 인증 · 일기 CRUD 4종 · React+Vite+Tailwind 스캐폴딩 · axios 인스턴스 · 로그인/회원가입/메인/일기 작성/일기 상세 화면(UI-001~005) · 인증 가드(ProtectedRoute) · 태그 API(PUT 전체 교체) · TagInput 컴포넌트 및 작성/상세 화면 연동 |
| **다음 작업** | M2 세션 2 — 검색 기능(제목/내용/태그) + 일기 수정 화면 (`ROADMAP.md` Milestone 2 참조) |
...
- Repository · `sagming40/onepage` (Public) · `main`에 PR #1~#6 병합 완료, 관련 feature 브랜치 전부 삭제
...
## 2026-08-06(오전 11 ~ 오후 8) — M1 완료: 일기 작성/상세 화면 구현 및 메인 화면 연결

**관련 마일스톤** · M1 (MVP 개발) → 완료

**한 일**

- 일기 작성 화면(UI-004) 구현 — 제목/내용/감정/위치 입력, 저장 시 상세 화면 이동
- 일기 상세 화면(UI-005) 구현 — 상세 조회, 삭제(확인창 포함)
- 메인 화면 감정 버튼 클릭 → 작성 화면 이동, 일기 카드 클릭 → 상세 화면 이동 연결
- API 레이어 확장 — `createDiary`/`getDiaryDetail`/`deleteDiary` 및 관련 타입(`CreateDiaryPayload`, `ApiMessageResponse`) 추가
- `feature/diary-ui` 브랜치에서 진행 → PR #5 생성 → `main` 병합, 브랜치 정리
- E2E 흐름 실전 검증 — 감정 선택 → 작성 → 저장 → 상세 조회 → 삭제 → 목록 미노출 → 삭제된 일기 404 확인까지 확인

**막혔던 점 / 트러블슈팅**

- **`onChange`를 `onSubmit`으로 오타** — 타이핑 시 제목 입력창에 값이 전혀 안 들어가는 증상. `value` prop만 있고 정상적인 `onChange`가 없어 React가 매 렌더링마다 state 값(빈 문자열)으로 덮어써서 발생. 콘솔 경고(`value prop ... without onChange handler`)로 확인 후 수정
  - 교훈: 문법적으로 유효한 이벤트 핸들러 이름이면 TS가 타입 에러를 잡아주지 못한다 (컴포넌트가 HTML 속성을 통째로 상속받는 구조라 `onSubmit`도 "존재하는 prop"이라 통과됨). `Math.cell` 오타와 같은 계열
- **지시 반영 중 `className` 통째로 누락** — "삼항연산자만 걷어내라"는 수정 지시를 처리하며 `className` 속성 자체를 삭제해버림. 컴파일 에러도, 기능 에러도 없어서 화면을 직접 봐야만 발견 가능한 유형
  - 교훈: 스타일링 관련 수정은 로직과 달리 컴파일러가 잡아주지 않으므로, 코드 검수만으로 끝내지 않고 반드시 실제 화면 렌더링까지 확인할 것
- **브라우저 bfcache로 인한 일시적 화면 불일치** — 일기 삭제 후 브라우저 "뒤로가기"로 상세 화면에 재진입하면, 서버에 재요청하지 않고 브라우저가 저장해둔 이전 스냅샷을 그대로 보여줘 삭제 전 상태가 잠깐 나타남. 코드 버그가 아니라 브라우저 표준 동작(Back-Forward Cache)이며, 새로고침이나 재조작 시 즉시 정상화됨을 확인 → M1 범위에서는 대응하지 않고 M5(최적화 및 개선)로 이관
- **디자인 토큰 오용** — 테두리 전용 색상 토큰(`text-line`, `#DEE2E6`)을 날짜/위치 텍스트에 사용해 글씨가 흐리게 보이는 문제 발견. `text-ink/60`(본문색 60% 불투명도)로 통일

**다음에 할 일**

- M2(핵심 기능 확장) 착수 — 사진 업로드 / 음악 기록 / 위치·태그 기능, 검색, 통계
- M2 이후 refreshToken 도입 검토 (M1 스코프 제외 결정 사항, 위 결정 기록 참조)

## 2026-08-12(오전 8 ~ 10) — M2 진행: 태그 기능(Backend API + Frontend UI) 구현 및 PR #6 병합

**관련 마일스톤** · M2 (핵심 기능 확장) → 진행 중 (태그 기능 완료, 세션 1)

**한 일**

- Diary-Tag N:M 관계를 다루는 첫 기능 — `tag.repository.ts`(connectOrCreate + 트랜잭션 기반 전체 교체), `tag.service.ts`(소유권 검증, 개수·길이·중복 검증), `tag.controller.ts`/`tag.routes.ts` 구현
- `PUT /diaries/:id/tags` 엔드포인트 신설 — 03_api-spec.md 8절의 POST/DELETE 개별 방식을 실제로는 전체 교체 방식으로 재설계 후 구현 (사유는 결정 기록 참조)
- `diary.repository.ts`의 `findById`에 `diaryTags.tag` include 추가, `diary.service.ts`에서 화면용 `tags: string[]` 형태로 가공
- Frontend — `TagInput` 컴포넌트(알약형 입력/삭제, 개수·중복 제한, 한글 IME 조합 방어), 작성 화면 연동(일기 생성 후 태그 별도 저장, 태그 저장 실패는 비차단 처리), 상세 화면 읽기 전용 표시
- PowerShell 정상/실패(404, 400) 케이스 검증 + 브라우저 E2E 검증(정상 흐름/개수 제한/중복 무시/한글 조합/무태그 저장) 완료
- `feature/tag-api` 브랜치에서 진행 → PR #6 생성 → `main` 병합, 브랜치 정리

**막혔던 점 / 트러블슈팅**

- **Prisma Unchecked/Relational 관계 표현 혼용** — `diaryId`(숫자 직행, Unchecked 방식)와 `tag: { connectOrCreate }`(관계 방식, Relational 방식)를 한 `create` 호출에 섞어 써서 타입 에러(TS2322) 발생 → `diary: { connect: { id: diaryId } }`로 양쪽 다 관계 방식으로 통일
  - 교훈: DiaryTag처럼 관계 필드가 여러 개인 모델은 한쪽만 관계 방식을 쓰면 안 되고, 같은 `data` 객체 안에서는 표현 방식을 통일해야 한다
- **Controller에서 성공 응답(`res.json`) 통째로 누락** — `tagService.replaceDiaryTags` 호출 후 `result`를 받아만 두고 응답을 안 보내는 상태로 커밋 직전까지 감. 컴파일 에러도, 즉각적인 런타임 에러도 없어 요청이 무한 대기 상태가 됨 — 검수 과정에서 발견
  - 교훈: "성공 경로에 실제 `res.json` 호출이 있는가"를 매 Controller 작성 후 습관적으로 확인할 것 (M1 트러블슈팅에서 나온 `className` 통째로 누락 사고와 같은 계열 — 컴파일러가 못 잡는 항목)
- PowerShell로 PUT/GET 응답을 확인할 때 배열 필드(`tags`)가 `System.Object[]`로 접혀 보여 실제 값 확인이 안 됨 → `$result.data.tags`처럼 필드를 따로 찍어서 펼쳐 확인

**다음에 할 일**

- M2 세션 2 착수 — 검색 기능(제목/내용/태그) + 일기 수정 화면
- (개선 후보) 태그 표시 순서가 입력 순서가 아니라 DB 내부 정렬(PK 순)을 따름 — 지금은 기능상 문제없으나, 자동완성/최근순 정렬 기능이 추가되는 시점에 `orderBy` 명시적으로 정하는 것을 함께 검토
...
<details>
<summary><b>2026-08-06 · UI-004 일기 작성 화면을 4항목(제목/내용/감정/위치)으로 축소한 이유</b></summary>

`04_ui-ux-design.md` UI-004 목업엔 제목/내용/감정/사진/음악/위치/태그 7항목이 명시되어 있으나, 실제 `POST /diaries` API가 받는 필드는 title/content/emotion/location 4개뿐임 (사진/음악/태그는 각각 별도 엔드포인트 — `03_api-spec.md` 6~8절).

**판단** · 사진/음악/태그를 화면에만 넣고 실제 저장 로직을 안 붙이면 "누르면 아무 일도 안 일어나는 버튼"이 되어 사용자(향후 실사용 시)에게 혼란을 준다. 반대로 지금 세 기능을 전부 붙이면 파일 업로드(Multer), 외부 URL 파싱, 다대다 관계(diary_tags) 처리까지 한 세션에 몰리게 되어, M1 완료 기준(회원가입→로그인→작성→조회)과 무관한 범위까지 스코프가 번진다.

**결정** · M1에서는 API가 실제로 지원하는 4항목만 구현하고, 사진/음악/태그는 관련 API와 함께 M2로 이관한다. `04_ui-ux-design.md`에 각주로 이관 사실을 남긴다.

</details>

<details>
<summary><b>2026-08-06 · UI-005 일기 상세 화면에서 수정 기능을 M1 범위에서 제외한 이유</b></summary>

`04_ui-ux-design.md` UI-005 목업엔 [수정][삭제] 버튼이 함께 명시되어 있고, `03_api-spec.md`에도 `PUT /diaries/{id}` API는 이미 구현되어 있음. 다만 이를 화면에 연결하려면 일기 작성 화면(UI-004)을 "새로 쓰기"와 "고쳐 쓰기" 두 모드로 분기해야 함 — 기존 값 프리필, 제출 시 POST/PUT 분기, 수정 완료 후 리다이렉트 등 UI-004 구현과는 별개의 설계가 필요함.

**판단** · ROADMAP M1 Frontend 작업 목록에는 애초에 "일기 수정 화면"이 항목으로 존재하지 않았고, M1 완료 기준(`회원가입 → 로그인 → 일기 작성 → 작성한 일기 조회`)에도 수정은 포함되지 않음. 삭제는 API 호출 한 번으로 끝나 부담이 적지만, 수정은 그 자체로 한 세션 분량의 설계가 필요한 별개 작업으로 판단.

**결정** · M1에서는 상세 화면에 삭제 버튼만 구현하고, 수정 기능은 M2 이후로 이관한다. `04_ui-ux-design.md`에 각주로 이관 사실을 남긴다.

</details>

<details>
<summary><b>2026-08-12 · 태그 API를 POST/DELETE 개별 방식 대신 PUT 전체 교체 방식으로 설계한 이유</b></summary>

`03_api-spec.md` 원안(M0 작성)엔 `POST /diaries/{id}/tags`(추가)와 `DELETE /diaries/{id}/tags/{tagId}`(제거)로 나뉘어 있었으나, 실제 화면 흐름(일기 작성 시 태그 여러 개를 한 번에 입력 후 저장)과 맞지 않았음.

**판단** · 태그 3개를 붙이려면 API를 3번 호출해야 하는데, 중간에 네트워크가 끊기면 일부 태그만 붙은 어중간한 상태가 남고 되돌릴 방법이 없다. "이 일기의 태그는 앞으로 이 배열이다"를 한 번에 선언하는 방식이면 트랜잭션 하나로 전부 성공하거나 전부 실패하게 만들 수 있고, 추후 일기 수정 화면(M2 세션 2)에서도 "추가분/삭제분을 따로 계산"할 필요 없이 최종 배열만 보내면 되어 재사용성도 높다.

**결정** · `PUT /diaries/{id}/tags`로 통일, `03_api-spec.md` 8절 및 엔드포인트 요약표 수정.

</details>

<details>
<summary><b>2026-08-12 · 태그 이름의 대소문자를 통합 처리하기로 한 이유</b></summary>

`onepage` DB의 콜레이션이 `utf8mb4_unicode_ci`(대소문자 미구분)이라, `Tag.name`에 걸린 `@unique` 제약상 `"React"`와 `"react"`는 같은 값으로 취급되어 두 번째 저장 시도가 Unique 제약 위반으로 실패하는 구조였음.

**판단** · 이를 해결하는 방법은 세 가지였음 — (A) 그대로 두어 먼저 등록된 표기가 이기게 함, (B) 전부 소문자로 강제 통일, (C) 콜레이션을 `_bin`으로 바꿔 대소문자를 구분함. (C)는 `React`/`react`가 별개 태그로 중복 생성되어 애초에 "같은 개념을 묶는다"는 태그의 목적에 어긋나므로 제외. (B)는 고유명사(`React`, `iOS` 등)가 전부 소문자로 표시되어 가독성이 떨어짐.

**결정** · (A) 그대로 둔다 — 즉 콜레이션을 변경하지 않고, 먼저 등록된 표기를 그대로 재사용한다. 태그의 목적 자체가 "같은 개념을 하나로 묶는 것"이므로, 표기가 통합되는 쪽이 오히려 의도에 부합한다고 판단.

</details>

<details>
<summary><b>2026-08-12 · 태그가 모든 일기에서 제거되어도(고아 태그) 삭제하지 않기로 한 이유</b></summary>

`replaceDiaryTags`로 특정 일기에서 태그 연결(`DiaryTag`)이 사라지면, `Tag` 테이블엔 그 태그가 그대로 남아 어떤 일기와도 연결되지 않은 상태(고아 태그)가 될 수 있음.

**판단** · 고아 태그를 자동으로 정리하려면 매번 "이 태그를 쓰는 다른 일기가 남아있는가"를 세는 쿼리가 하나 더 필요해 성능 비용이 든다. 반면 태그 하나가 차지하는 저장 공간은 무시할 수준이며, 남겨두면 추후 태그 자동완성 기능(M2 이후)의 후보 목록으로 재사용할 수 있다.

**결정** · 고아 태그를 별도로 정리하지 않고 그대로 유지한다. 추후 태그 개수가 많아져 정리가 필요해지면 그때 배치 작업 등으로 별도 처리한다.
